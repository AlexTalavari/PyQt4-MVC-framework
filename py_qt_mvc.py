...
class WidgetCodeGenerator(object):
    ...
    def __init__(self):
        ...
        self.ordered_funcs = [
            ...
            {'the_func'             : self.get_ctrl_class_code,
             'func_single_only'     : True,
             'func_header'          : '\n\n\n###########################\n# controllers\\'+self.fname+'Controller.py #\n###########################\n'},
    ...
    def get_view_class_code(self):
        return ''.join(['from PyQt4 import QtGui\nfrom gen.'+self.fname+' import Ui_'+self.fname+' \n\nclass '+self.fname+'(QtGui.QMainWindow):'])
    ...
    def get_ctrl_class_code(self):
        return ''.join(['from PyQt4 import QtGui\n\nclass '+self.fname+'Controller(object):\n\n',
    ...
    def get_app_code(self):
        return ''.join(['import sys\n',
                        ...
                        'from model.Model import '+self.fname+'Model\n',
                        'from controllers.'+self.fname+'Controller import '+self.fname+'Controller\n',
                        'from views.'+self.fname+'View import '+self.fname+'View\n\n',
                        'class App(QtGui.QApplication):\n',
                        '    def __init__(self, sys_argv):\n',
                        '        super(App, self).__init__(sys_argv)\n',
                        '        self.model = '+self.fname+'Model()\n',
                        '        self.main_ctrl = '+self.fname+'Controller(self.model)\n',
                        '        self.main_view = '+self.fname+'View(self.model, self.main_ctrl)\n',
                        '        self.main_view.show()\n\n',
    ...
    def process_file(self, argv):
        ...
        if len(argv) > 1:
            if os.path.exists(argv[1]):
                widget_names = []
                self.fname = os.path.basename(argv[1])
        ...
        # write to new file
        with open('genMvc/'+self.fname + '.py', 'w') as thefile:
